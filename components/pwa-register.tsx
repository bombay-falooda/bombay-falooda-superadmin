...
export function PwaRegister() {
  ...
  useEffect(() => {
    ...
    navigator.serviceWorker
      .register("/sw.js")
      .then((reg) => console.log("Superadmin PWA SW registered:", reg.scope))
      .catch((err) => console.warn("Superadmin PWA SW error:", err));
    ...
    if (Notification.permission === "default") {
      ...
      // Auto-register FCM Web Push token
      void requestNotificationPermission();
    }

    // Listen for foreground FCM push notifications
    const unsubscribe = onForegroundMessage((payload) => {
      console.log("🔔 Foreground notification in Superadmin:", payload);
    });
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-bold text-white leading-tight">Enable Push Notifications</h4>
          <p className="text-xs text-slate-300 mt-1 leading-relaxed">
            Get real-time audio and pop-up alerts for new orders, franchise sync, and critical system events.
          </p>
